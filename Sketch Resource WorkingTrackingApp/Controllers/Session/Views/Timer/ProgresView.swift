...
extension TimerView {
    final class ProgresView: UIView {
        func drawProgres(with percent: CGFloat) {
            ...
            defaultCircleLayer.strokeColor = Resouces.Colors.inactive.cgColor
            ...
            let dotPaht = UIBezierPath()
            dotPaht.move(to: dotPoint)
            dotPaht.addLine(to: dotPoint)
            
            let bigDotLayer = CAShapeLayer()
            bigDotLayer.path = dotPaht.cgPath
            ...
            let barsPaht = UIBezierPath(arcCenter: center, radius: barsRadius, startAngle: startAngle, endAngle: endAngle, clockwise: true)
            
            let barsLayer = CAShapeLayer()
            barsLayer.path = barsPaht.cgPath
            ...
            (1...9).forEach { _ in
                ...
                ? Resouces.Colors.active.cgColor : Resouces.Colors.inactive.cgColor
